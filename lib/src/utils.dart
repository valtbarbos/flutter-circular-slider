...
double calculateAngle(
  ...
  if (counterClockwise) {
    return calculateAngleCounterClockwise(startAngle: startAngle, angleRange: angleRange, selectedAngle: selectedAngle, previousAngle: previousAngle);
  }
...
double calculateAngleCounterClockwise({@required double startAngle, @required double angleRange, @required selectedAngle, @required previousAngle}) {
  double angle = radiansToDegrees(selectedAngle);
...
double angleToValue(
  ...
  var value = percentageToValue(angleToPercentage(angle, angleRange), min, max);
  if (range.isNotEmpty) {
    var rangeValue = null;
    for (var i = 0; i < range.length; i++) {
      var x = range[i];
      var y = range[(i + 1 == range.length ? i : i + 1)];
      if (value >= x && value <= y) {
        rangeValue = value <= (x + x * 0.5) ? x : y;
      }
    }
    value = rangeValue;
  }
